...
var Sheets = function(sheet_id, timeout) {
    this.id = sheet_id;
...
Sheets.prototype.setTimeout = function(timeout) {
    this.timeout = timeout;
};

Sheets.prototype.load = function(load_callback) {
    var self = this;

    async.series({
        meta: function(meta_callback) {
            self.sheet.getSpreadsheetData(Sheets.META, function(err, data) {
                if (err) {
                    return meta_callback(err);
                }

                self.meta = data;
                self.last_updated = self.sheet.last_updated;
...
Sheets.prototype.toJSON = function() {
    return this.content;
};

Sheets.prototype.get = function(collection, identifier) {
    if (!this.ready) {
        return null;
    }
...
Sheets.prototype.loadData = function(collection, type, callback) {
    var self = this;
    ...
    this.sheet.getSpreadsheetData(collection, function(err, data) {
        ...
        if (type === 'hash') {
            ...
            data = Sheets.extractKeyValue(data);
            ...
            // TODO: Break up object on .'s
            self.content[self.meta[collection].collection] = Sheets.extractArray(data);
        }
...
Sheets.extractKeyValue = function(keyvalues) {
...
Sheets.extractArray = function(values) {
    var array = [];
...
Sheets.ValidationError = ValidationError;
Sheets.TimeoutError = TimeoutError;
